...
@section('title', $categoryName ?? 'Todos os Produtos')

@section('content')
<div class="container mt-5">
    <h1 class="mb-4 section-title">{{ $categoryName ?? 'Todos os Produtos' }}</h1>

    @if($products->isNotEmpty())
        <div class="row">
            @foreach($products as $product)
            {{-- Card do produto aqui, usando $product->name, $product->image, $product->price, etc. --}}
            {{-- E $product->category->name se quiser mostrar a categoria --}}
                ...
                <div class="card product-card h-100 shadow-sm">
                    ...
                    <div class="card-body d-flex flex-column">
                        ...
                        <p class="card-text text-muted small mb-2">
                            @if($product->category)
                                <a href="{{ route('categories.show', $product->category->slug) }}" class="text-muted">{{ $product->category->name }}</a>
                            @endif
                        </p>
